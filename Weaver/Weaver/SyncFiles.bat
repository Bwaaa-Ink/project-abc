...
copy /Y "TrixxInjection.Framework\Attributes\Configurator.cs"  "TrixxInjection.Fody\AUTOGENERATED_Configurator.g.cs"
copy /Y "TrixxInjection.Framework\Writer.cs"  "TrixxInjection.Fody\AUTOGENERATED_Writer.g.cs"

setlocal EnableDelayedExpansion

rem --- set source & destination
set "srcRel=TrixxInjection.Framework\Attributes\Markers.cs"
set "src=%~dp0%srcRel%"
...
for /f "usebackq delims=" %%A in ("%~dp0classes.tmp") do (
  >>"%dst%" echo         static partial void %%A^(CustomAttribute attribute, EntityUnion attr^);
)

>>"%dst%" echo.

for /f "usebackq delims=" %%A in ("%~dp0classes.tmp") do (
  >>"%dst%" echo         public static void %%A_Exposer^(CustomAttribute attribute, EntityUnion attr^) =^> %%A^(attribute, attr^);
)
